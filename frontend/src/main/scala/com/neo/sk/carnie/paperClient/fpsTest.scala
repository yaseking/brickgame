...
class fpsTest(order: String, webSocketPara: WebSocketPara) {
  ...
  def gameRender(): Double => Unit = { _ =>
    ...
    println(s"la2: ${1000/(curTime - lastTime2)}")
    lastTime2 = curTime
...
//          drawGameImage(myId, data, offsetTime)
  ...
  def drawGameImage(uid: String, data: Data4TotalSync, offsetTime: Long): Unit = {
    ...
    drawGame.drawGrid(uid, data, offsetTime, grid, currentRank.headOption.map(_.id).getOrElse(myId), scale)
//     drawGame.test()
