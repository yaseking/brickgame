...
trait EventDistributeTable {
  import org.seekloud.utils.DBUtil.driver.api._

  class EventDistributeTable(tag: Tag) extends Table[EventDistribute](tag, "EVENT_DISTRIBUTE") {
    val distributor = column[String]("DISTRIBUTOR", O.PrimaryKey)
  ...
  protected val eventDistributeTableQuery = TableQuery[EventDistributeTable]
