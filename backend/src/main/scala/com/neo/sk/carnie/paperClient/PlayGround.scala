...
//          grid.stateEveryFrame = if(grid.stateEveryFrame.isEmpty) Some(Snapshot(grid.grid, grid.snakes, List(JoinEvent(id)))) //玩家加入储存快照,更新用户加入信息
//          else Some(Snapshot(grid.grid, grid.snakes, JoinEvent(id) :: grid.stateEveryFrame.get.joinOrLeftEvent))   //for replay
//
...
//            val grid = roomMap(roomId)._2
//            grid.stateEveryFrame = if(grid.stateEveryFrame.isEmpty) Some(Snapshot(grid.grid, grid.snakes, List(LeftEvent(id)))) //玩家离开储存快照,更新用户离开信息
//            else Some(Snapshot(grid.grid, grid.snakes, LeftEvent(id) :: grid.stateEveryFrame.get.joinOrLeftEvent))
//
//            grid.removeSnake(id)
//            if (newUserNum <= 0) {
//              roomMap -= roomId
...
//                if(grid.stateEveryFrame.isEmpty) grid.stateEveryFrame = Some(Snapshot(grid.grid, grid.snakes, Nil))
//
//              }
//            }
//          }
