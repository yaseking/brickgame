...
trait SlickTables {
  ...
  case class rUserInRecord(userId: String, recordId: Long, roomId: Int, nickname: Option[String] = None)
  /** GetResult implicit for fetching rUserInRecord objects using plain SQL queries */
  implicit def GetResultrUserInRecord(implicit e0: GR[String], e1: GR[Long], e2: GR[Int], e3: GR[Option[String]]): GR[rUserInRecord] = GR{
    prs => import prs._
  ...
  class tUserInRecord(_tableTag: Tag) extends profile.api.Table[rUserInRecord](_tableTag, "user_in_record") {
    ...
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(userId), Rep.Some(recordId), Rep.Some(roomId), nickname).shaped.<>({r=>import r._; _1.map(_=> rUserInRecord.tupled((_1.get, _2.get, _3.get, _4)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
